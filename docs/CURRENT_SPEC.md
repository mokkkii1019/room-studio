...
- **ジャンル・品目はどちらも「指定しない」が既定**。何も選ばずフリーワードやメーカーだけで検索できる（指示032）。
  - 送るキーは `any`（両方とも指定なし）/ `any_furniture` `any_appliance` `any_daily`（ジャンルのみ指定）。
  - `any` はフリーワード／メーカーだけで楽天全体を検索し、入力が何も無いときだけインテリアジャンル（`100804`）を既定の枠にする（楽天APIは検索条件が最低1つ必要）。
  - ジャンルのみ指定は代表語（家具/家電/インテリア）＋そのジャンル配下の品目語の**和集合**（`_provider_base.match_words`）で絞る。ふるさと納税（家電はさらに景品・目録）は `NGKeyword` で除外。
  - ジャンル未指定のとき、品目ドロップダウンは全ジャンルをジャンル見出し付きで並べる（ジャンルを先に選ばなくても品目を直接選べる）。メーカー一覧も3ジャンル分を重複なしで表示。
...
- **法務ページ**: `/about`（運営者情報）・`/privacy`（プライバシーポリシー）・`/tokushoho`（特商法表記）を静的生成。運営者名・連絡先・住所は環境変数（`OPERATOR_NAME`/`OPERATOR_CONTACT`/`OPERATOR_ADDRESS`）で差し替え可。連絡先メールは `CONTACT_EMAIL` で与えると法務3ページ（`/about`・`/privacy`・`/tokushoho`）すべてに同じ値が表示される（未設定なら行ごと非表示。スパム対策として表示は数値文字参照、`mailto:` の href はクライアント側で組み立てるため、HTMLのソースに素のアドレスも `mailto:` も現れない）。全ページに「アフィリエイト広告（PR）を含む」旨と「商品情報提供：楽天ウェブサービス」のクレジットを表示。

...
LP刷新で各LPに「画像スロット」を実装済み。**ファイルを `lp-assets/` に置くだけ**で自動的に表示されます。**未配置の枠は公開ページに表示されません**（開発用の指示テキストや空の箱は出さず、テキスト主体のクリーンな状態になります）。ファイルを置くと、その枠が自動で表示されます（before/afterは2枚そろったときだけ表示）。

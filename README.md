...
2. **表面（壁/床/天井）** … 「表面を追加」→ 範囲を指定 → **色** / **素材** を変更。**既定で不透明に完全被覆（上塗り）**し、質感・陰影は保ったまま置換（下地を透かしたい場合は「強さ」を下げる）。範囲指定は3通り:
   ...
   - **画像から素材を抽出** … 素材セクションの「画像から素材を抽出」で写真（木目・布・石など）を読み込むと、中央正方形を切り出し**継ぎ目なし（タイル化）の素材**を生成して一覧に追加・適用（`makeSeamless`：ハーフオフセット＋中央シーム羽化ヒーリング）。抽出素材は**localStorageに自動保存（再読込で復元）**＋**プロジェクト保存(.json)にも含まれる**（`serializeUserMats`/`registerUserMaterial`/`loadUserMatsLocal`）。

...
#### 楽天ウェブサービス規約の遵守メモ（法的助言ではありません）
- **収益化は楽天アフィリエイトのみ**（規約 第10条1項(4)(5)）。別アフィリ網・機能課金で楽天データから収益化は不可。
- WS利用部分は**楽天サイトへのリンク必須／楽天以外へのリンク禁止**（第8条4項）→ 収集画像の「購入」は楽天itemUrlで適合。
- **クレジット表示**（第13条）→ 収集UIに「商品情報提供：楽天ウェブサービス」を表示。
- **取得画像の複製/改変/共有保管に注意**（第10条(7)(9)・第6条IP）→ 楽天商品画像は「閲覧・選択・購入導線」用に留め、recolor/台形補正/背景除去などの“改変”やリスト保存での画像配布は避けるのが安全。重い編集はユーザ自身がアップした画像で行う想定。
- **特定少数のみアクセス可能な環境での利用は要明示許可**（第10条(10)）→ 製品化時は公開アプリにするのが安全（localhost個人利用はグレー）。
- アクセス回数等の制限を順守（第7条3項）。

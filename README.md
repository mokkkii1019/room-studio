...
### ネットから家具画像を収集 ✅ 実装済み（2026-06）
家具タブの「**ネットから収集**」で、**種類**（椅子/ダイニングテーブル/ソファ/ベッド/ローテーブル/ランプシェード/テーブルランプ/カーペット/観葉植物）＋**テイスト自由記述**（例: 北欧）＋**枚数**（最大80・既定50）を指定すると、**Openverse**（CCライセンス・APIキー不要）から画像を収集して家具ライブラリに追加。
- `server.py` の `/collect`（Openverse検索・匿名は20件/ページのためページング）＋ `/imgproxy`（外部画像を同一オリジンで中継＝キャンバス非汚染で背景除去/書き出し可）。`server.py` の起動が必要。
- 収集画像はライブラリに入り、クリック/ドラッグで配置。**「リストを保存／読込」**で一覧を `.json` 化（`exportLib`/`importLib`）。
- 取得は原画像URLを中継（Openverse APIのレート制限を消費しない）。商用利用時は各画像のライセンス確認が必要。

...
### 家具店から画像を収集（楽天市場API） ✅ 実装済み（2026-06）
家具タブの「**家具店から収集（楽天市場）**」で、**種類**（椅子/ダイニングテーブル/ソファ/ベッド/ローテーブル/ランプシェード/テーブルランプ/カーペット/観葉植物）＋**テイスト自由記述**（例: 北欧）＋**枚数**（最大80・既定50）を指定すると、**楽天市場 商品検索API**（家具ジャンル=インテリア・寝具・収納 `genreId=100804`）から商品写真を収集してライブラリに追加。商品写真なので人物・風景のノイズが少ない。
- なぜスクレイピングしないか: IKEA/ニトリ等のHTML直取得は規約違反になりやすく壊れやすい。商品APIなら**画像＋購入リンク**が公式に取れ、マネタイズにも直結。
...
- 注: IKEA日本は楽天/Amazonにほぼ出店がなくこのルートでは取得不可（ニトリ等は可）。Amazon PA-API / Yahoo!ショッピングAPI も同様の方式で追加可能。

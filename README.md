...
- **ART OF BLACK（`source=artofblack`）** … 公式サイト(STORES)はボット対策が強く直接取得不可のため、**ART OF BLACKの楽天店**（`shopCode=artofblack`）を楽天API経由で取得（`_collect_rakuten`）。楽天の新API（`openapi.rakuten.co.jp/ichibams/...20260401`）を使用し、**`RAKUTEN_APP_ID`（UUID）＋`RAKUTEN_ACCESS_KEY`** が必要。サーバーは「許可されたWebサイト」に一致する **Referer/Origin ヘッダ**を付与（既定 `https://github.com/`、`RAKUTEN_REFERER` で変更可）。

...
for (const fn of ['candGray', 'candPercentile', 'candTextScore', 'candGutterScore', 'candVividScore', 'candSeamScore', 'candPlaceFeat']) {
  if (!src.includes('function ' + fn)) { console.error('missing ' + fn); process.exit(2); }
}
const m = new Function(src + '; return {candTextScore, candGutterScore, candVividScore, candSeamScore, candPlaceFeat};')();
...
for (const e of exp) {
  ...
  if (!ok) {
    ...
    console.log(`MISMATCH ${e.file} glyphs ${t.glyphs}/${e.glyphs} ` +
      ...
      `fgBorder ${p.fgBorder}/${e.fg_border}`);
  }
}
